...
export const Jobs = () => {
  ...
  const leftFilter = (jobsList) => {
    ...
    const leftFilArr = jobsList.filter((item) => {
      return item.category.toLowerCase() === category.toLowerCase() && (salary === 0 || parseInt(item.salary.replace(/[^0-9]/g, '')) >= salary)
  })
    setJobs(leftFilArr)
  }
              ...
              {!isMobile &&  <div className='filter1  flex  flex-col'>
                  <div className='flex justify-start  flex-col filter  '>
                    ...
                    <div className='flex flex-col gap-4 w-2/3 pt-5'>
                      <button onClick={() => leftFilter(allJobs)} className='blueCol px-1 py-1 text-xs'>Apply Filter</button>
