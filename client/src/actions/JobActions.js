...
export const createJobPost = (jobData) => async (dispatch) => {
    try{
        ...
        const {data} = await axios.post("http://localhost:8000/api/v1/create/job",jobData,config) ;        
...
export const getSingleJob = (id) => async (dispatch) => {
    try{
        ...
        const {data} = await axios.get(`http://localhost:8000/api/v1/job/${id}`) ;
...
export const saveJob = (id) => async (dispatch) => {
    try{
        ...
        const {data} = await axios.get(`http://localhost:8000/api/v1/saveJob/${id}`,config) ;
...
export const getSavedJobs = () => async (dispatch) => {
    try{
        ...
        const {data} = await axios.get("http://localhost:8000/api/v1/getSavedJobs",config) ;
